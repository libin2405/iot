...
const RealTimeDetection: React.FC = () => {
  ...
  const connectToServer = () => {
   ...
   newSocket.on('sms_status', (status) => {
     setSmsStatus(status);
   });

   newSocket.on('sms_test_result', (result) => {
  ...
  const testSMS = () => {
    if (socket) {
      ...
      socket.emit('test_sms', phoneNumber ? { phone_number: phoneNumber } : {});
    }
  };
  ...
  // Cleanup on unmount
  useEffect(() => {
    // Get SMS status when component mounts
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="text-xs text-gray-500 mt-1">
                  Leave empty to test all configured contacts
                </div>
              </div>
              <button
                onClick={testEmail}
